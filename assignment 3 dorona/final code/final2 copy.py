...
# log transformation
D = np.log(D)
data = pd.DataFrame({'Year': years, 'Disease_Spread': D})

# Make two disjoint sets, "Training set" and "Testing set," with 80% of training data and 20% of testing data
train_data, test_data, train_target, test_target = train_test_split(
    data['Year'].values.reshape(-1, 1), data['Disease_Spread'].values, test_size=0.2)
...
plt.scatter(data['Year'], data['Disease_Spread'], label='Actual data')
plt.plot(data['Year'], reg.predict(
    data['Year'].values.reshape(-1, 1)), label='Linear Regression')
plt.legend()
