...
def log_regression(start_year, end_year, pct_train):
    train_time, test_time, train_d, test_d = prepare_data(
        start_year, end_year, pct_train)

    # take the logarithm of the target values
    train_log_d = np.log(train_d)
    test_log_d = np.log(test_d)

    # fit the linear regression model
    reg = LinearRegression().fit(train_time.reshape(-1, 1), train_log_d)

    # predict the logarithm of the target values for the training and testing sets
