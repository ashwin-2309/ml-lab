...
class MNISTClassifier:
    ...
    def compare_accuracies(self, X_test, y_test):
        ...
        print('Task 1: PCA Test Accuracy (k=3):', accuracy_score(
            y_test, self.knn3.predict(self.pca.transform(X_test))))
        print('Task 1: PCA Test Accuracy (k=5):', accuracy_score(
            y_test, self.knn5.predict(self.pca.transform(X_test))))
        print('Task 2: LDA Test Accuracy (k=3):', accuracy_score(
            y_test, self.knn3.predict(self.lda.transform(X_test))))
        print('Task 2: LDA Test Accuracy (k=5):', accuracy_score(
            y_test, self.knn5.predict(self.lda.transform(X_test))))
        print('Task 3: Autoencoder Test Accuracy (k=3):', accuracy_score(
            y_test, self.knn3.predict(self.autoencoder(torch.Tensor(X_test)).detach().numpy()[:, :9])))
        print('Task 3: Autoencoder Test Accuracy (k=5):', accuracy_score(
            y_test, self.knn5.predict(self.autoencoder(torch.Tensor(X_test)).detach().numpy()[:, :9])))
